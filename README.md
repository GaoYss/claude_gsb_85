...
| 整改跟踪 | 整改状态机流转 + 整改流水（措施 / 进展 / 验收 / 销号），逾期自动标记；列表支持多选后批量指派责任人、批量催办，整批原子生效并逐条留痕 |
...
**批量指派与批量催办**（`backend/app/services/hazard_service.py` 的 `batch_assign / batch_remind`）：

- 隐患列表支持跨页多选，可批量指派整改责任人、批量催办；每次指派 / 催办都会逐条写入整改流水
  （记录类型「指派责任人」/「催办提醒」，详情页时间轴可见），批次本身落 `hazard_batch_operation` 表留痕。
- **整批原子生效**：提交前先整批校验（隐患不存在、已销号等），任一不满足则整批回滚，
  返回 409 并在报文中明确「整批未生效」且逐条列明原因，不存在只改一半的情况。
- **幂等**：前端每次发起批量操作生成唯一 `request_id`，同一批次重复提交（双击、网络重试）
  直接重放首次结果（`already_processed=true`），不会产生重复催办 / 指派记录；
  `request_id` 被不同批次复用会被拒绝（409）。
- **数量对账**：响应携带 `processed_count` 与 `hazard_ids`，前端与跨页勾选数量核对后展示结果。
- 已销号隐患不参与批量操作；「登记 / 指派 / 催办」三类流水由系统生成，人工追加记录时不可选。
- 责任人候选由 `GET /hazards/assignee-candidates?keyword=` 聚合历史责任人 / 发现人 / 巡查人，
  服务端模糊检索 + 限量返回，候选人多时下拉仍可检索，也允许直接输入新姓名。
- 批量操作成功后前端会刷新列表，并使总览页统计缓存失效，首页待办数量立即同步。
...
| `hazard_batch_operation` | 批量操作批次（幂等键、类型、处理条数、快照） | 独立审计表 |
...
| GET | `/hazards/assignee-candidates` | 整改责任人候选检索（聚合历史责任人 / 发现人 / 巡查人） |
| POST | `/hazards/batch-assign` | 批量指派整改责任人（整批原子、幂等） |
| POST | `/hazards/batch-remind` | 批量催办（整批原子、幂等） |
...
```bash
cd backend
python -m pytest            # 39 个接口用例：台账 CRUD、编号生成、巡查结论推导、状态机、逾期、统计、批量操作
```
